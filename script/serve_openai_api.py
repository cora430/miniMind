...
class ChatRequest(BaseModel):
    ...
    def get_open_thinking(self) -> bool:
        ...
        if self.chat_template_kwargs:
            return self.chat_template_kwargs.get("open_thinking", False) or self.chat_template_kwargs.get("enable_thinking", False)
        return False
...
def parse_response(text):
    ...
    if think_match:
        reasoning_content = think_match.group(1) # 只匹配括号里的
        re.sub(r'<think>.*?</think>\s*', "", text, flags=re.DOTALL)
    elif "</think>" in text:
        parts = text.split("</think>", 1)
        reasoning_content = parts[0].split()
        text = parts[1].split() if len(parts) == 2 else ""
    tool_calls = []
    for i, m in enumerate(re.findall(r'<tool_call>(.*?)</tool_call>', text, re.DOTALL)):
        try:
            ...
            pass
    if tool_calls:
        re.sub(r'<tool_call>(.*?)</tool_call>', "", text, flags=re.DOTALL)
    return text, reasoning_content, tool_calls or None
    ...
    try:
        ...
        while True:
            ...
            if not thinking_ended:
                ...
                new_r = full_text[emitted:]
                if new_r:
                        yield json.dumps({"choices": [{"delta": {"content": new_r}}]}, ensure_ascii=False)
                        emitted = len(full_text)
