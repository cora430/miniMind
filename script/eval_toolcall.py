...
from trainer.trainer_utils import get_model_params, setup_seed,Logger
...
def init_model(args):
    ...
    if "model" in args.load_from:
        model = MiniMindForCausalLM(MiniMindConfig(hidden_size=args.hidden_size, use_moe=args.use_moe, num_hidden_layers=args.num_hidden_layers, ))
...
def parse_tool_call_from_text(content):
    ...
    matches = re.findall(pattern=pattern, string=content, flags=re.DOTALL)
    calls = []
    if not matches:
        return None
...
def execute_tool(call, arguments=None):
    ...
    try:
        ...
        return {"error": f"执行失败：{str(e)[:80]}"}
def generate(model, tokenizer, messages, tools, args):
    input_text = tokenizer.apply_chat_template(
        messages, add_generation_prompt=True, tokenize=False, open_thinking=False, tools=tools)
    inputs = tokenizer(
    ...
    if args.show_speed:
        Logger(f"\nspeed : {(len(outputs[0]) - len(inputs['input_ids'][0]))/(time.time() - st):.2f} tokens / s")
    return response
...
def run_case(prompt, tools, args, model=None, tokenizer=None, client=None):
    ...
    while True:
        if args.backend == "local":
            ...
            content, tool_calls = chat_api(client, messages, tools, args, bool(args.stream), )
        if not tool_calls:
            break
        tool_calls = [{
        ...
        messages.append({"role": "assistant", "content": content}) if args.backend == "local" else messages.append({"role":"assistant", "content": content, "tool_calls": [{"id": tc["id"], "type": "function", "function": {"name": tc["name"], "arguments": tc["arguments"]}} for tc in tool_calls]})
        ...
        for tc in tool_calls:
            ...
            print(f'✅ [Tool Called]: {json.dumps(result, ensure_ascii=False).strip()}')
...
def main():
    ...
    parser.add_argument('--load_from', default='/root/autodl-tmp/miniMind/model', type=str, help="模型加载路径（model=原生torch权重，其他路径=transformers格式）")
    parser.add_argument('--save_dir', default='/root/autodl-tmp/miniMind/checkpoints', type=str, help="模型权重目录")
    ...
    parser.add_argument('--show_speed', action="store_true", help="显示decode速度（tokens/s）")
    ...
    }for case in TEST_CASES] if input_mode == 0 else iter(lambda: {"prompt":input('💬: '), "tools":TOOLS, "tool_names": TOOL_NAMES_ALL}, {"prompt":"", "tools":TOOLS, "tool_names": TOOL_NAMES_ALL})
    for case in cases:
        ...
        if input_mode == 0:
            ...
            print(f'💬: {case["prompt"]}')
        run_case(case["prompt"], case["tools"], args, model, tokenizer, client)
        print('\n' + '-' * 50 + '\n')
