...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight', default='full_sft', type=str, help="权重名称前缀（pretrain, full_sft, rlhf, reason, ppo_actor, grpo, spo, full_sft_t2t）")
    ...
    input_mode = int(input("[0]自动输入 [1]手动输入"))
    streamer = TextStreamer(tokenizer, skip_prompt=True, skip_special_tokens=True)
    prompt_iters = prompts if input_mode == 0 else iter(lambda: input("💬: "), "")
    for prompt in prompt_iters:
        setup_seed(2026)
        ...
        if args.show_speed == 1: 
            print(f"speed is {gen_token_num / (time.time() - st):.2f} tokens/s \n\n") 
        else: 
            print("\n\n")
